...
public class PurchaseOrderController {
    ...
    public String purchaseOrders(
        ...
        int autoTotalPages = autoGeneratedOrders.getTotalPages();
        int autoStartPage = Math.max(0, page1 - maxVisiblePages / 2);
        int autoEndPage = Math.min(autoTotalPages - 1, autoStartPage + maxVisiblePages - 1);

        model.addAttribute("autoOrders", autoGeneratedOrders.getContent());
        model.addAttribute("autoTotalPages", autoTotalPages);
        model.addAttribute("autoCurrentPage", autoGeneratedOrders.getNumber());
        model.addAttribute("autoStartPage", autoStartPage);
        model.addAttribute("autoEndPage", autoEndPage);

        // 아래쪽 테이블: 발주완료 상태인 항목
        Page<PurchaseOrderDTO> completedOrders = purchaseOrderService.getOrdersByStatus("발주완료", page2, size2);
        int completedTotalPages = completedOrders.getTotalPages();
        int completedStartPage = Math.max(0, page2 - maxVisiblePages / 2);
        int completedEndPage = Math.min(completedTotalPages - 1, completedStartPage + maxVisiblePages - 1);
    ...
    public String viewPurchaseOrder(@PathVariable String purchaseOrderCode, Model model) {
        ...
        return "procurement/purchaseOrder"; // HTML 템플릿 파일 이름
    }
}